...
export async function deposit(accountNo: string, sum: number) {
    try {
        ...
        if (!result) {
            throw new Error('Account not found') // Express WILL send a vague error to the front if no account is found
        }
...
export async function loan(accountNo: string, sum: number, interest: number, noOfPayments: number) {
    try {
        await mongoose.connect(process.env.MONGODB_URI as string)
